...
namespace Unity.MLAgents
{
    ...
    /// </summary>
    internal class MoAgentVectorActuator : VectorActuator
    {
        public MoAgentVectorActuator(IActionReceiver actionReceiver,
    ...
    public partial class MoAgent : MonoBehaviour, ISerializationCallbackReceiver, IActionReceiver, IHeuristicProvider
    {
        IPolicy m_Brain;
        ...
        // Added to track multiple objectives
        //[SerializeField]
        public static int noOfObjectives = 2;
        ...
        //float m_Reward;
        List<float> m_RewardList = new List<float>();
        /// Represents the group reward the agent accumulated during the current step.
        //float m_GroupReward;
        List<float> m_GroupRewardList = new List<float>();
        /// Keeps track of the cumulative reward in this episode.
        // Im going to leave this as a float as its the total rewards for the episode 
        ...
        // Keeps track of all steps for incrementing the weight during training
        int m_Steps;
        
        /// How many steps until the weight is incremented during training
        ...
        internal event Action<MoAgent> OnAgentDisabled;
        ...
        public void LazyInitialize()
        {
            ...
            m_Info = new MoAgentInfo();
            sensors = new List<ISensor>();

            //Debug.Log(noOfObjectives);


            //init lists 
            m_Info.reward = new List<float>();
            m_Info.groupReward = new List<float>();
            m_RewardList = new List<float>();
            m_GroupRewardList = new List<float>();
            m_CumulativeReward = 0;
            //Init the lists so they have values to start from
            for (int i = 0; i < noOfObjectives; i++)   
            {
                m_RewardList.Add(0);
                m_GroupRewardList.Add(0);
        ...
        void NotifyAgentDone(DoneReason doneReason)
        {
            ...
            //Make sure only the current rewards exist
            m_Info.reward.Clear();
            m_Info.groupReward.Clear();


            foreach (var i in m_RewardList)
            {
                m_Info.reward.Add(i);
            }

            foreach (var i in m_GroupRewardList)
            {
                m_Info.groupReward.Add(i);
            }


            //m_Info.reward = m_RewardList;
            ...
            if (doneReason != DoneReason.Disabled)
            {
                ...
                m_CompletedEpisodes++;
                //for(var i = 0; i < m_RewardList.Count; i++)
                //{
                UpdateRewardStats();
                //}
            }
        ...
        public void SetReward(int objective, float reward)
        {
            Utilities.DebugCheckNanAndInfinity(reward, nameof(reward), nameof(SetReward));

            try {

                if(m_Steps == (m_WeightSteps * weightCounter))
                {
                   m_Weight += 1f;
                   weightCounter++;
                }

                //modify the reward based on the weight
                ...
                if (objective == 0)
                {
                    ...
                    if (weight < 3f && reward > 0f) //Dont reward the first objective at all if the weight is low
                    {
                        reward = 0f;
                    }
                ...
                {
                    if (weight > 7f) //Dont reward the second objective if the weight is above 0.7f
                    {
                        reward = 0f;
                    }
                ...
                for (int r = 0; r < noOfObjectives; r++)
                {
                    
                    if (r == objective)
                    {
                        reward *= weight; 
                        m_CumulativeReward += (reward - m_RewardList[objective]);
                        m_RewardList[r] = reward;
                        m_GroupRewardList[r] = reward;
        ...
        public void AddReward(int objective, float increment)
        {
            Utilities.DebugCheckNanAndInfinity(increment, nameof(increment), nameof(AddReward));
            m_RewardList[objective] += increment;
            m_CumulativeReward += increment;
        }

        internal void SetGroupReward(int objective, float reward)
        {
            Utilities.DebugCheckNanAndInfinity(reward, nameof(reward), nameof(SetGroupReward));
            m_GroupRewardList[objective] = reward;
        }

        internal void AddGroupReward(int objective, float increment)
        {
            Utilities.DebugCheckNanAndInfinity(increment, nameof(increment), nameof(AddGroupReward));
            m_GroupRewardList[objective] += increment;
        }
        ...
        void InitializeActuators()
        {
            ...
            var param = m_PolicyFactory.BrainParameters;
            m_VectorActuator = new MoAgentVectorActuator(this, this, param.ActionSpec);
        ...
        void SendInfoToBrain()
        {
            ...
            //all rewards
            //m_info lists need to be cleared first or the list of rewards on the
            //python end just keeps growing
            m_Info.reward.Clear();
            m_Info.groupReward.Clear();

            foreach (var rew in m_RewardList)
            {
                m_Info.reward.Add(rew);
            }
            
            foreach(var rew in m_GroupRewardList)
            {
                m_Info.groupReward.Add(rew);
            }
        ...
        void SendInfo()
        {
            ...
            if (m_RequestDecision)
            {
                ...
                {
                    m_RewardList[i] = 0f;
                    m_GroupRewardList[i] = 0f;
                }
                
                m_RequestDecision = false;
                m_Steps += 1; //Keep track of total steps for incrementing weight during trainng
            }
        }
